...
var flush_stats = function elastic_flush(ts, metrics) {
  ...
  //ts = "2019-09-25T18:53:47.883Z"
  var indexDate = new Date();
  var yr = indexDate.getUTCFullYear()
  var mo = indexDate.getUTCMonth()
  var date = indexDate.getUTCDate()
  var hr = indexDate.getUTCHours()
  if (indexDate.getUTCMinutes() < 10) {
    var min = '00'
  }
  else {
    var min = indexDate.getUTCMinutes()
  }
  if (indexDate.getUTCSeconds() < 10) {
    var sec = '00'
  }
  else {
    var sec = indexDate.getUTCSeconds()
  }

  //ts = '' +yr+ '-10-' +date+ 'T' +hr+ ':' +min+ ':' +sec+ '.000Z'
  //moment(ts).format('MMMM MM YYYY h:mm:ss');
  lg.log('ES payload::::::::::::::::::::::\n\n\n');
  lg.log(ts) //'' +yr+ '-' +mo+ '-' +date+ 'T' +hr+ ':' +min+ ':' +sec+ '.883Z')
  lg.log('\n\n\n');
